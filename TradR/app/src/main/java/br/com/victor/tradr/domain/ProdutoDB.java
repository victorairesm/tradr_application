...
public class ProdutoDB extends SQLiteOpenHelper {
    ...
    public long save(Produto produto) {
        ...
        try {
            ...
            values.put("desc", produto.descricao);
    ...
    public int delete(Produto produto) {
        ...
        try {
            ...
            int count = db.delete("carro", "_id=?", new String[]{String.valueOf(produto.id)});
    ...
    public Produto getById(Long id) {
        ...
        try {
            // "select * from carro where tipo=?"
            Cursor c = db.query("carro", null, "_id = " + id , null, null, null, null);
            List<Produto> produto = toList(c);
            Produto produto = produto.size() > 0 ? produto.get(0) : null;
            return produto;
        } finally {
            db.close();
        }
    ...
    private List<Produto> toList(Cursor c) {
        ...
        if (c.moveToFirst()) {
            do {
                ...
                produtos.id = c.getLong(c.getColumnIndex("_id"));
                produtos.nome = c.getString(c.getColumnIndex("nome"));
                produtos.descricao = c.getString(c.getColumnIndex("desc"));
                produtos.urlInfo = c.getString(c.getColumnIndex("url_info"));
                produtos.urlFoto = c.getString(c.getColumnIndex("url_foto"));
                produtos.urlVideo = c.getString(c.getColumnIndex("url_video"));
                produtos.latitude = c.getString(c.getColumnIndex("latitude"));
                produtos.longitude = c.getString(c.getColumnIndex("longitude"));
                produtos.tipo = c.getString(c.getColumnIndex("tipo"));
